...
def get_projections(inputline):
    if re.search("project", inputline, re.IGNORECASE):
        ...
        if ("certainty" not in project_cols.split(",")):
            project_cols+=", certainty"
        return project_cols
    else:
        return '*'
...
def process_join (cur, relation):
    ...
    temp_table="_certainty_step2"
    result_table="_certainty_step3"
    ...
    while (relations):
        ...
        execute_query(cur, 'ALTER TABLE {} RENAME COLUMN certainty2 TO certainty'.format(result_table))
        current=result_table
    return result_table
        
def process_query_certainty(inputline, cur):
    print ("Processing certainty")
